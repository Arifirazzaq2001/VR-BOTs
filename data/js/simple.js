...
let typee = m.messageStubType
let participants = m.messageStubParameters
switch (typee) {
case 27:
case 31:
this.emit('group-add', { m, typee, participants })
break
case 28:
case 32:
this.emit('group-leave', { m, typee, participants })
break
case 40:
case 41:
case 45:
...
this.emit('call', {
typee, participants,
isGroup: typee == 45 || typee == 46,
isVideo: typee == 41 || typee == 46
})
break
}
})
...
try {
let m = json[2][0][2]
if (m.message && m.message.protocolMessage && m.message.protocolMessage.mtype == 0) {
...
m = await this.prepareMessageMedia(message, mtype, options)
break
case 'orderMessage':
...
let mtype = await FileType.fromBuffer(data) || {
...
async fakeReply2(jid, message, mtype, sender, message2, type2, options = {}, options2 = {}, remoteJid) {
...
return this.prepareMessage(jid, message, mtype, { quoted, ...options })
}
...
if (m.quoted) {
let mtype = Object.keys(m.quoted)[0]
m.quoted = m.quoted[mtype]
if (['productMessage'].includes(mtype)) {
mtype = Object.keys(m.quoted)[0]
m.quoted = m.quoted[mtype]
}
if (typeof m.quoted === 'string') m.quoted = { text: m.quoted }
m.quoted.mtype = mtype
...
if (m.quoted.url) m.quoted.download = (mtype = 'buffer') => conn.downloadM(vM, mtype)
/**
...
if (m.msg.url) m.download = (mtype = 'buffer') => conn.downloadM(m, mtype)
