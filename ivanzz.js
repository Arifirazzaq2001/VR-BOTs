...
module.exports = exifCreate = (packname, authorname, filename) => {
...
ytsnya = ytsnya.all;
let namaLagu = ytsnya[0].title
let ytdlnya = ytdl(ytsnya[0].videoId, {
quality: 'highestaudio',
});
ffmpeg(ytdlnya)
.audioBitrate(320)
.save('./database/sampah/' + namaLagu + '.mp3')
.on('end', async () => {
saveMedia = new ID3Writer(fs.readFileSync('./database/sampah/' + namaLagu + '.mp3'));
await Vanz.sendMessage(from, Buffer.from(saveMedia.arrayBuffer), audio, {mimetype: Mimetype.mp4Audio, ptt: true, quoted: vnz});
fs.unlinkSync('./database/sampah/' + namaLagu + '.mp3')
});
break
